'''
python xgboost_eval_main_dataset_mclass.py > output/xgboost_eval_main_dataset_mclass-output.txt
'''
import argparse
import pandas as pd
import numpy as np
...
# train_df, ttune_df, train_label, ttune_label = train_test_split(train_df, train_label, test_size=0.2, random_state=72)
...
for DEPTH in range(10,12): # (3,25):
    ...
    P, R, T = precision_recall_curve(ttune_label, ttune_pred)
    ...
    for idx in range(len(ttune_pred)):
        if ttune_pred[idx] < T[F1index]:
            ...
            ttune_pred[idx] = 1
    f1_tune = f1_score (ttune_label , ttune_pred) # (?) .detach() )
    '''    
    ...
    f1_test = f1_score (ttest_label , ttest_pred) # (?) .detach() )
